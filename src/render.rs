...
impl RenderMaterial {
    ...
    pub fn texture(mut self, texture_name: &str) -> Result<Self, io::Error> {
        ...
        self.texture = Some(image::open(texture_name).or_else(|_| {
            Err(io::Error::new(
                io::ErrorKind::Other,
                "texture image file load failed",
            ))
        })?);
        Ok(self)
    }
...
impl RenderMaterialInterface for RenderMaterial {
    ...
    fn lookup_texture(&self, uv: (f32, f32)) -> RenderColor {
        let (u, v) = uv;
        if let Some(image::DynamicImage::ImageRgb8(ref texture)) = self.texture {
            match self.texture_filter {
                TextureFilter::Nearest => {
                    let pixel = *texture.get_pixel(
                        imod((u * texture.width() as f32) as i32, texture.width() as i32) as u32,
                        imod(
                            (v * texture.height() as f32) as i32,
                            texture.height() as i32,
                        ) as u32,
                    );
                    return RenderColor {
                        r: pixel[0] as f32 / 256.,
                        g: pixel[1] as f32 / 256.,
                        b: pixel[2] as f32 / 256.,
                    };
                }
                TextureFilter::Bilinear => {
                    ...
                    let pixel = [
                        ...
                        scale_pixel(
                            (1. - fu) * fv,
                            *texture.get_pixel(iu, umod(iv + 1, texture.height())),
                        ),
                        scale_pixel(
                            fu * (1. - fv),
                            *texture.get_pixel(umod(iu + 1, texture.width()), iv),
                        ),
                        scale_pixel(
                            fu * fv,
                            *texture.get_pixel(
                                umod(iu + 1, texture.width()),
                                umod(iv + 1, texture.height()),
                            ),
                        ),
                    ]
                    .iter()
        ...
        match self.pattern {
            ...
            RenderPattern::RepeatedGradation => RenderColor::new(
...
impl Into<std::io::Error> for DeserializeError {
    fn into(self) -> std::io::Error {
        std::io::Error::new(
            ...
            "Deserialize error: ".to_string() + &self.s,
        )
    }
}
...
impl RenderSphere {
    ...
    pub fn new(material: Arc<RenderMaterial>, r: f32, org: Vec3) -> RenderObject {
        RenderObject::Sphere(RenderSphere::new_raw(material, r, org))
    }

    fn new_raw(material: Arc<RenderMaterial>, r: f32, org: Vec3) -> RenderSphere {
        RenderSphere {
            material,
            r,
            org,
    ...
    fn deserialize(
        ren: &RenderEnv,
        serial: &RenderSphereSerial,
    ) -> Result<RenderObject, DeserializeError> {
                ...
                ren.materials
                    ...
                    .ok_or_else(|| {
                        DeserializeError::new(&format!(
                            "RenderSphere couldn't find material {}",
                            serial.material
                        ))
                    })?
                    .clone(),
...
impl RenderObjectInterface for RenderSphere {
    ...
    fn get_diffuse(&self, position: &Vec3) -> RenderColor {
        self.material
            .lookup_texture(self.material.get_uv(&(*position - self.org), self.uvmap))
    }
...
impl RenderFloor {
    #[allow(dead_code)]
    #[allow(clippy::new_ret_no_self)]
    pub fn new(material: Arc<RenderMaterial>, org: Vec3, face_normal: Vec3) -> RenderObject {
        RenderObject::Floor(RenderFloor::new_raw(material, org, face_normal))
    }

    pub fn new_raw(material: Arc<RenderMaterial>, org: Vec3, face_normal: Vec3) -> RenderFloor {
        RenderFloor {
            material,
            org,
            face_normal,
    ...
    fn deserialize(
        ren: &RenderEnv,
        serial: &RenderFloorSerial,
    ) -> Result<RenderObject, DeserializeError> {
        Ok(RenderObject::Floor(
            Self::new_raw(
                ren.materials
                    .get(&serial.material)
                    .ok_or_else(|| {
                        DeserializeError::new(&format!(
                            "RenderFloor couldn't find material {}",
                            serial.material
                        ))
                    })?
                    .clone(),
...
impl RenderEnv {
    ...
    pub fn serialize(&self) -> Result<String, std::io::Error> {
        let mut sceneobj = Scene {
            ...
            objects: self
                .objects
                .iter()
                .map(|o| o.get_interface().serialize())
                .collect(),
        };
        for object in &self.objects {
            ...
            sceneobj
                .materials
                .insert(material.name.clone(), material.serialize());
        }
    ...
    pub fn deserialize(&mut self, s: &str) -> Result<(), DeserializeError> {
        ...
        let mm: Result<HashMap<_, _>, DeserializeError> = sceneobj
            .materials
            .into_iter()
            .map(|m| Ok((m.0, Arc::new(RenderMaterial::deserialize(&m.1)?))))
            .collect();
...
pub fn render(
    ...
    let process_line = |iy: i32, point_middle: &mut dyn FnMut(i32, i32, RenderColor)| {
        for ix in 0..ren.xres {
            ...
            let mut eye: Vec3 = Vec3::new(
                /* cast ray direction vector? */
                1.,
    ...
    if thread_count == 1 {
        ...
        crossbeam::scope(|scope| {
            use std::io::Write;
            let log_file = Arc::new(Mutex::new(std::io::BufWriter::new(
                std::fs::File::create("timing.csv").unwrap(),
            )));
            log_file
                .lock()
                .unwrap()
                ...
                .map(|tid| {
                    ...
                    scope.spawn(move |_| -> WorkerResult {
                        ...
                        loop {
                            ...
                            log_file.lock().unwrap().write(
                                format!("{}, {}, {}\n", tid, iyy, ren.start.elapsed().as_nanos())
                                    .as_bytes(),
                            );
                            if ren.yres <= iyy {
                                break;
...
pub fn render_frames(
    ...
    for (n, frame) in ren.camera_motion.0.iter().enumerate() {
        ...
        for i in 0..(frame.duration / frame_step) as i32 {
            ...
            println!(
                "Rendering frame {} / {}, v0: {},{}",
                accum_frame, total_frames, v0.x, v0.y
            );
            ren.camera.position =
                hermite_interpolate(f, &prev_camera.position, &frame.camera.position, &v0, &v1);
...
fn shading(ren: &RenderEnv, idx: usize, n: &Vec3, pt: &Vec3, eye: &Vec3, nest: i32) -> RenderColor {
        ...
        (
            ...
            *pt + (ren.light * eps),
            if 0 != pn {
                ...
                if reflection_incidence > 0.0 {
                    reflection_incidence.powi(pn)
                } else {
                    0.0
                }
            } else {
    ...
    let (k1, k2) = {
        ...
        if ren.use_raymarching {
            let RaymarchSingleResult {
                iter, travel_dist, ..
            } = raymarch_single(ren, &reflected_ray, &ray, Some(&ren.objects[idx]));
            if FAR_AWAY <= travel_dist
                || MAX_ITER <= iter
                || 0.
                    < ren.objects[idx]
                        .get_interface()
    ...
    if nest < ren.max_refractions && 0. < o.get_material().get_transparency() {
        ...
        let fc2 = {
            ...
            (if ren.use_raymarching {
                ...
                &mut ray,
                nest,
                Some(&ren.objects[idx]),
                if sp < 0. { OUTONLY } else { INONLY },
            )
        };
...
fn raytrace(
    ...
    loop {
        ...
        if t < std::f32::INFINITY {
            ...
            if 0 == (RIGNORE & flags) {
                ret_color.r += face_color.r * fcs.r;
                fcs.r *= ks.r;
            }
            if 0 == (GIGNORE & flags) {
                ret_color.g += face_color.g * fcs.g;
                fcs.g *= ks.g;
            }
            if 0 == (BIGNORE & flags) {
                ret_color.b += face_color.b * fcs.b;
                fcs.b *= ks.b;
            }
            if idx == 0 {
                break;
...
fn distance_estimate(ren: &RenderEnv, vi: &Vec3, ig: Option<&RenderObject>) -> (f32, usize, f32) {
...
fn raymarch(
    ...
    loop {
        ...
        let RaymarchSingleResult {
            ...
            min_dist,
            ..
        } = raymarch_single(ren, &pos, eye, ig);
    ...
    if let Some(glow_effect) = ren.glow_effect {
        let factor = if min_min_dist == std::f32::INFINITY {
            1.
        } else {
            1. + (0. + glow_effect * (0.99f32).powf(min_min_dist))
        };
        RenderColor::new(
            factor * ret_color.r,
            factor * ret_color.g,
